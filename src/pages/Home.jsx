...
import {
  ...
  ChevronRight, Star, TrendingUp, Eye
} from 'lucide-react'
import { getElections, getCandidates } from '../lib/api'
...
function StatCard({ label, value, icon: Icon, colorClass, delay, started }) {
  const count = useCounter(value, 1800, started)
  return (
    <div
      className="flex flex-col items-center justify-center p-6 rounded-2xl bg-white/10 backdrop-blur border border-white/20 hover:bg-white/15 transition-all duration-300"
    ...
    >
      ...
      <p className="text-4xl font-bold text-white mb-1 tabular-nums">{count.toLocaleString()}</p>
      <p className="text-primary-100 text-sm font-medium text-center">{label}</p>
    </div>
  )
}
...
export default function Home() {
    ...
    ;(async () => {
      try {
        ...
        for (const e of elecs || []) {
          const r = await getCandidates(e.id)
          totalCandidates += (r.data || []).length
        }
        try {
          const { data: s } = await axios.get('/api/admin/stats')
          setStats({ voters: s.total_voters || 0, elections: s.total_elections || (elecs||[]).length, candidates: s.total_candidates || totalCandidates, votes: s.total_voted || 0 })
        } catch {
          setStats({ voters: 0, elections: (elecs||[]).length, candidates: totalCandidates, votes: 0 })
        }
      } catch { /* silent */ }
  ...
  return (
    <div className="min-h-screen bg-slate-50 overflow-x-hidden">
      ...
      {/* ══ LIVE STATS ══════════════════════════════════ */}
      <section className="bg-gradient-to-br from-primary-700 via-primary-600 to-indigo-700 pt-0 pb-20" ref={statsRef}>
        <div className="max-w-5xl mx-auto px-4">
          <p className={`text-center text-primary-200 text-sm font-semibold uppercase tracking-widest mb-6 transition-all duration-700 ${statsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        ...
        {loading ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {elections.map((e, i) => (
              <div
                key={e.id}
                className={`card card-glow hover:shadow-lg transition-all duration-500 hover:-translate-y-1 group cursor-pointer ${electionsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${i * 0.12}s` }}
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 transition-colors duration-300 ${
                  ['bg-primary-100 group-hover:bg-primary-500', 'bg-emerald-100 group-hover:bg-emerald-500', 'bg-amber-100 group-hover:bg-amber-500'][i % 3]
                }`}>
                  <Vote className={`h-5 w-5 transition-colors duration-300 ${
                    ['text-primary-600 group-hover:text-white', 'text-emerald-600 group-hover:text-white', 'text-amber-600 group-hover:text-white'][i % 3]
                  }`} />
                </div>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="font-semibold text-slate-800 leading-snug">{e.title}</h3>
                  <span className={`shrink-0 ${e.is_active ? 'badge-green' : 'badge-gray'}`}>
        ...
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((step, i) => (
              <div
                key={i}
                className={`text-center group transition-all duration-700 ${stepsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
              ...
              >
                ...
                <div className="relative mx-auto w-16 h-16 mb-5">
                  ...
                  <span className="absolute -top-2 -right-2 w-6 h-6 bg-primary-500 rounded-full text-white text-xs font-bold flex items-center justify-center">
                    {i + 1}
                  </span>
                </div>
                ...
                {i < steps.length - 1 && (
                  <div className="hidden lg:block absolute top-8 left-full w-full">
                    <div className={`h-px bg-gradient-to-r from-primary-500 to-transparent transition-all duration-1000 ${stepsVisible ? 'w-full opacity-100' : 'w-0 opacity-0'}`}
          ...
          <div className={`text-center mt-14 transition-all duration-700 delay-700 ${stepsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <Link to="/register" className="group inline-flex items-center gap-2 bg-primary-500 hover:bg-primary-400 text-white font-bold px-8 py-4 rounded-xl transition-all shadow-lg hover:shadow-primary-500/30 hover:-translate-y-0.5">
              Get Started Now
              <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
            </Link>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-24" ref={featuresRef}>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f, i) => (
            <div
              key={f.title}
              className={`card card-glow group hover:shadow-xl transition-all duration-500 hover:-translate-y-1 ${featuresVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
              style={{ transitionDelay: `${i * 0.1}s` }}
            >
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center mb-4 transition-all duration-300 ${f.color[0]} group-hover:${f.color[1]} group-hover:scale-110`}>
                <f.icon className={`h-5 w-5 transition-colors duration-300 ${f.color[2]} group-hover:text-white`} />
              </div>
              <h3 className="font-semibold text-slate-800 mb-2 group-hover:text-primary-600 transition-colors duration-300">{f.title}</h3>
      ...
      <section className="bg-gradient-to-br from-emerald-500 to-teal-600 py-20 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          ...
          <h2 className="text-3xl font-bold text-white mb-4" style={{ fontFamily: 'Sora, sans-serif' }}>
            100% Transparent Election Results
          </h2>
          ...
          <Link to="/results" className="group inline-flex items-center gap-2 bg-white text-emerald-600 font-bold px-8 py-3.5 rounded-xl hover:bg-emerald-50 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5">
      ...
      <section className="max-w-5xl mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div key={t.name} className={`border-2 rounded-xl p-5 text-center hover:scale-105 hover:shadow-md transition-all duration-300 cursor-default ${t.color}`}
        ...
        <section className="bg-gradient-to-br from-primary-600 to-indigo-700 py-24 relative overflow-hidden" ref={ctaRef}>
          ...
          <div className={`relative max-w-3xl mx-auto px-4 text-center transition-all duration-700 ${ctaVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <div className="flex items-center justify-center gap-4 flex-wrap">
              <Link to="/register" className="group flex items-center gap-2 bg-white text-primary-600 font-bold px-8 py-4 rounded-xl hover:bg-primary-50 transition-all shadow-lg hover:shadow-xl hover:-translate-y-1">
                Create Account
                <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
              </Link>
